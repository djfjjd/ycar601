...
회사 표시는 [src/site-config.js](src/site-config.js) 한 곳에서 관리합니다. 현재 대표자는 `정미현`, 사업자번호는 `221-81-49254`이며, 주소는 제공되기 전까지 `정보 등록 필요`로 표시됩니다. 공식 로고가 준비되기 전까지 `윤카` 텍스트 기반 임시 아이콘을 사용합니다.
