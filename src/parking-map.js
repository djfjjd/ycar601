...
export function renderParkingMap(layout,spots,visibleIds=new Set(spots.map(spot=>spot.id)),options={}){
  ...
  const allRows=Array.from({length:layout.rows},(_,index)=>index+1),hasToggle=Boolean(layout.collapseBeforeRow||layout.collapsedVisibleRows),collapsed=hasToggle&&!options.expanded,showCoordinates=!collapsed&&!layout.hideCoordinates,showColumnHeaders=showCoordinates&&!layout.hideColumnHeaders,showRowLabels=showCoordinates||Boolean(layout.rowLabels),headerRows=layout.hideHeaderRow?0:1,collapsedRows=layout.collapsedVisibleRows||allRows.filter(row=>row>=layout.collapseBeforeRow),visibleRows=hasToggle?(collapsed?collapsedRows:allRows):(layout.visibleRows||allRows),columns=options.expanded&&layout.expandedColumns?layout.expandedColumns:layout.columns,gridRowByActual=new Map(visibleRows.map((row,index)=>[row,index+1+headerRows])),cells=[];
