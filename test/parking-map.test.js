...
test('6층은 7행까지만 접고 D08~D09와 D10~D11을 각각 활성 주차면으로 병합한다',()=>{
  ...
  assert.match(expanded,/grid-row:9\/span 2[^>]+aria-label="D08 빈 자리"/);
  assert.doesNotMatch(expanded,/aria-label="D09 빈 자리"/);
  ...
  assert.match(expanded,/aria-label="06번기둥"/);
