...
test('차량현황판은 지정된 네 담당자 탭과 정렬 순서를 사용한다',()=>{
  assert.match(main,/const MANAGER_ORDER=\['대표님','박이사님','이부장님','임회장님','오승한','담당자 미지정'\]/);
  assert.match(main,/categories=\[\['all','전체'\],\['대표님','대표님'\],\['박이사님','박이사님'\],\['이부장님','이부장님'\],\['임회장님','임회장님'\],\['오승한','오승한'\]\]/);
});
